...
class CocoKeypoints(CocoDataset):
    def __init__(self,
                 ...
                 joints_generator,
                 transforms=None):
        super().__init__(cfg.get("data", "root"),
                         dataset_name,
                         cfg.get("data", "data_format"))

        if cfg.get("data", "with_center"):
            assert cfg.get("data", "num_joints") == 18, 'Number of joint with center for COCO is 18'
        else:
            assert cfg.get("data", "num_joints") == 17, 'Number of joint for COCO is 17'

        self.num_scales = self._init_check(heatmap_generator, joints_generator)
        self.num_joints = cfg.get("data", "num_joints")
        self.with_center = cfg.get("data", "with_center")
        self.num_joints_without_center = self.num_joints - 1 \
            if self.with_center else self.num_joints
        self.scale_aware_sigma = cfg.get("data", "scale_aware_sigma")
        self.base_sigma = cfg.get("data", "base_sigma")
        self.base_size = cfg.get("data", "base_size")
        self.int_sigma = cfg.get("data", "int_sigma")
