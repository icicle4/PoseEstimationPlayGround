    ...
    for i in range(params.num_joints):
        ...
        if i == 0 or len(joint_dict) == 0:
            ...
            for row, col in pairs:
                if (
                    row < num_added and col < num_grouped
                    ...
                    key = tags[row][0]
                    joint_dict.setdefault(
                        key, np.copy(default_)
                    )[idx] = joints[row]
...
class Params:
    def __init__(self, configer):
        self.num_joints = configer.get("data", "num_joints")
        self.max_num_people = configer.get("data", "max_num_people")

        self.detection_threshold = configer.get("test", "detection_threshold")
        self.tag_threshold = configer.get("test", "tag_threshold")
        self.use_detection_val = configer.get("test", "use_detection_val")
        self.ignore_too_much = configer.get("test", "ignore_too_much")

        if configer.get("data", "with_center") and configer.get("test", "ignore_center"):
            self.num_joints -= 1

        if configer.get("data", "with_center") and not configer.get("test", "ignore_center"):
            self.joint_order = [
...
class HeatmapParser:
    def __init__(self, configer):
        self.params = Params(configer)
        self.tag_per_joint = configer.get("model", "tag_per_joint")
        self.pool = torch.nn.MaxPool2d(
            configer.get("test", "nms_kernel", 1, configer.get("test", "nms_padding"))
        )
    ...
    def refine(self, det, tag, keypoints):
        ...
        :param det:
        :param tag:
        :param keypoints: numpy.ndarray of size (17, 4) if not flip, last dim is (x, y, det score, tag_score)
        :return:
        """
    ...
    def parse(self, det, tag, adjust=False, refine=False):
