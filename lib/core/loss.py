...
class MultiLossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_joints = cfg.get("model", "num_joints")
        self.num_stages = cfg.get("loss", "stages")
        ...
        self.heatmaps_loss = \
            nn.ModuleList(
                [
                    ...
                    for with_heatmaps_loss in cfg.get("loss", "with_heatmaps_loss")
                ]
            )
        self.heatmaps_loss_factor = cfg.get("loss", "heatmaps_loss_factor")

        self.ae_loss = \
            nn.ModuleList(
                [
                    AELoss(cfg.get("loss", "ae_loss_type")) if with_ae_loss else None
                    for with_ae_loss in cfg.get("loss","with_ae_loss")
                ]
            )
        self.push_loss_factor = cfg.get("loss", "push_loss_factor")
        self.pull_loss_factor = cfg.get("loss", "pull_loss_factor")
